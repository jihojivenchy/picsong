...
여기에 더해 **프롬프트 실험실**이라는 진단용 화면을 한동안 앱에 넣고 굴렸다. 프롬프트·시드를 바꿔가며 실기기에서 배치 생성해보는 개발용 도구로, **§4의 실증 목록이 전부 여기서 나왔다.** 처음부터 "원인 규명이 끝나면 제거한다"는 조건으로 만든 화면이라, 진단이 끝난 뒤 제거했다.
...
> 모델 변환 환경을 재현할 때는 **`numpy<2` 고정이 필수**다.
