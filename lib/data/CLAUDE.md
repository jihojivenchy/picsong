...
| `dtos/` | 도메인과 무관한 data 계층 내부 전용 응답(예: 토큰) | 도메인/UI 노출, 비즈니스 로직 |
...
* **DTO 예외:** 응답이 도메인과 무관하고 **data 계층 안에서만 쓰이다 사라지는** 경우에만 `dtos/`에 DTO를 둔다. (예: 토큰 응답 → 키체인/로컬 DB에 바로 저장.) DTO는 도메인/presentation으로 넘기지 않는다.
* **DTO 규약(간단):** PascalCase + `DTO` 접미사, 필드 `final`, `fromJson`으로 파싱.
...
* **모델 정의 금지:** Service 파일 내부에 엔티티/DTO 클래스를 정의하지 않는다. 모델은 각각 `lib/domain/entities/` 또는 `lib/data/dtos/`에 둔다.
